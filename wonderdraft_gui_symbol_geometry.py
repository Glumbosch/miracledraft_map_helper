...
class Godot3VariantFileParser:
    ...
    def __init__(
        ...
        self._progress_step = max(1024 * 1024, payload_length // 250 if payload_length else 1)
        self._next_progress = 0

    def _report_progress(self, *, force: bool = False) -> None:
        if self.progress is None:
            return
        if force or self.position >= self._next_progress:
            self.progress(self.position, self.payload_length)
            self._next_progress = self.position + self._progress_step
...
def decode_store_var_file(
    ...
    with path.open("rb", buffering=1024 * 1024) as handle:
        ...
        parser = Godot3VariantFileParser(handle, path, payload_length, progress=progress)
        result = parser.parse()
        parser._report_progress(force=True)
...
class GCPFStreamingWriter:
    ...
    def __init__(
        ...
        *,
        ...
        self.written = 0
        self.progress = progress
        self._progress_step = max(1024 * 1024, uncompressed_size // 250 if uncompressed_size else 1)
        self._next_progress = 0
    ...
    def write(self, data: bytes | bytearray | memoryview) -> int:
        ...
        while start < total:
            ...
            start += take
            self.written += take
            if self.progress is not None and self.written >= self._next_progress:
                self.progress(self.written, self.uncompressed_size)
                self._next_progress = self.written + self._progress_step
    ...
    def finish(self) -> int:
        ...
        if self.progress is not None:
            self.progress(self.uncompressed_size, self.uncompressed_size)
        return final_size
...
def write_wonderdraft_map_streaming(
    ...
    *,
    ...
    temp_handle.close()
    writer = GCPFStreamingWriter(
        temp_path, total_size, block_size, compressed=compressed, progress=progress
    )
    try:
...
class GodotTextParser:
    ...
    def make_constructor(name: str, args: list[Any], pos: int) -> GDValue:
        ...
        if name == "PoolByteArrayRef":
            if not args or not isinstance(args[0], str):
                raise FormatError(f"PoolByteArrayRef requires a path string at character {pos}")
            length = int(args[1]) if len(args) > 1 else 0
            return GDValue(name, (args[0], length))
        if name == "Object":
            if len(args) != 2 or not isinstance(args[0], str) or not isinstance(args[1], dict):
                raise FormatError(f"Object requires a class name and property dictionary at character {pos}")
            return GDValue("Object", {"class": args[0], "properties": args[1]})
        if name == "PoolIntArray":
...
class LazyGodotTextParser:
    ...
    def __init__(
        ...
        self.cursor = 0
        self.progress = progress
        self._progress_step = max(256 * 1024, len(text) // 250 if text else 1)
        self._next_progress = 0
    ...
    def _report_progress(self, *, force: bool = False) -> None:
        if self.progress is None:
            return
        if force or self.cursor >= self._next_progress:
            self.progress(self.cursor, len(self.text))
            self._next_progress = self.cursor + self._progress_step
...
def parse_godot_text(
    text: str,
    progress: Callable[[int, int], None] | None = None,
) -> Any:
    return LazyGodotTextParser(text, progress=progress).parse()


# ------------------------------- Image helpers -------------------------------

def prepare_editable_tree(
    value: Any,
    path: tuple[str, ...] = (),
    images: OrderedDict[str, GDValue] | None = None,
    binary_blobs: OrderedDict[str, GDValue] | None = None,
) -> tuple[Any, OrderedDict[str, GDValue], OrderedDict[str, GDValue]]:
    """Replace large disk-backed payloads with compact editable placeholders.

    Images retain the familiar ``.mask.png``/``.ground.png`` style placeholders.
    Any other PoolByteArray is represented as ``PoolByteArrayRef(path, length)``
    and restored byte-for-byte when the map is saved. This avoids both the
    ``DiskByteArray object is not iterable`` error and accidental multi-gigabyte
    text serialization.
    """
    if images is None:
        images = OrderedDict()
    if binary_blobs is None:
        binary_blobs = OrderedDict()
    joined = ".".join(path)

    if image_object_info(value) is not None:
        images[joined] = value
        leaf = path[-1] if path else "image"
        return f".{leaf}.png", images, binary_blobs

    if isinstance(value, GDValue):
        if value.type_name == "PoolByteArray":
            source = value.value
            if isinstance(source, DiskByteArray):
                binary_blobs[joined] = value
                return GDValue("PoolByteArrayRef", (joined, len(source))), images, binary_blobs
            return value, images, binary_blobs
        if value.type_name == "Object":
            props, images, binary_blobs = prepare_editable_tree(
                value.value.get("properties", OrderedDict()),
                path + ("properties",), images, binary_blobs,
            )
            return GDValue("Object", {
                "class": value.value.get("class", "Object"),
                "properties": props,
            }), images, binary_blobs
        return value, images, binary_blobs
    ...
    if isinstance(value, dict):
        ...
        for key, child in value.items():
            converted, images, binary_blobs = prepare_editable_tree(
                child, path + (str(key),), images, binary_blobs
            )
            out[key] = converted
        return out, images, binary_blobs

    if isinstance(value, list):
        out_list: list[Any] = []
        for index, child in enumerate(value):
            converted, images, binary_blobs = prepare_editable_tree(
                child, path + (str(index),), images, binary_blobs
            )
            out_list.append(converted)
        return out_list, images, binary_blobs

    return value, images, binary_blobs


def restore_external_payloads(
    value: Any,
    images: OrderedDict[str, GDValue],
    binary_blobs: OrderedDict[str, GDValue],
    path: tuple[str, ...] = (),
) -> Any:
    joined = ".".join(path)
    if joined in images:
        return images[joined]
    if joined in binary_blobs:
        return binary_blobs[joined]
    if isinstance(value, GDValue) and value.type_name == "Object":
        return GDValue("Object", {
            "class": value.value.get("class", "Object"),
            "properties": restore_external_payloads(
                value.value.get("properties", OrderedDict()), images, binary_blobs,
                path + ("properties",),
            ),
        })
    if isinstance(value, dict):
        return OrderedDict(
            (key, restore_external_payloads(child, images, binary_blobs, path + (str(key),)))
            for key, child in value.items()
        )
    if isinstance(value, list):
        return [
            restore_external_payloads(child, images, binary_blobs, path + (str(index),))
            for index, child in enumerate(value)
        ]
    if isinstance(value, GDValue) and value.type_name == "PoolByteArrayRef":
        raise FormatError(
            f"Cannot restore PoolByteArrayRef at {joined!r}; its backing payload is unavailable"
        )
    return value
...
def find_images(value: Any, path: tuple[str, ...] = ()) -> OrderedDict[str, GDValue]:
    ...
    _, images, _ = prepare_editable_tree(value, path)
    return images


def replace_images_with_names(value: Any, images: OrderedDict[str, GDValue], path=()) -> Any:
    # Compatibility helper retained for older scripts. New code uses
    # prepare_editable_tree so non-image disk payloads are handled as well.
    editable, _, _ = prepare_editable_tree(value, path, OrderedDict(images), OrderedDict())
    return editable


def restore_images(value: Any, images: OrderedDict[str, GDValue], path=()) -> Any:
    return restore_external_payloads(value, images, OrderedDict(), path)
...
def iter_godot_text(value: Any) -> Iterator[str]:
    ...
    if isinstance(value, GDValue) and value.type_name == "PoolByteArrayRef":
        ref_path, length = value.value
        yield f"PoolByteArrayRef( {to_godot_text(str(ref_path))}, {int(length)} )"
        return
    if isinstance(value, GDValue) and value.type_name == "Object":
        yield "Object( "
        yield to_godot_text(str(value.value.get("class", "Object")))
        yield ", "
        yield from iter_godot_text(value.value.get("properties", OrderedDict()))
        yield " )"
        return
    if isinstance(value, list):
        if not value:
        ...
        if isinstance(pixels, DiskByteArray):
            with pixels.path.open("rb", buffering=1024 * 1024) as source:
                ...
                for y in range(height):
                    ...
                    if progress is not None and (y % report_step == 0 or y + 1 == height):
                        progress((y + 1) * row_size, base_size)
        else:
...
class AssetResolver:
    ...
    def asset_info(self, texture: str | None) -> AssetInfo | None:
        ...
        metadata = self._metadata_for_path(path) or {}

        # Wonderdraft renders a symbol at the PNG/SVG's intrinsic dimensions
        # multiplied by the map record's ``scale``.  The metadata ``radius`` is
        # the symbol's logical/selection radius and is copied into new records;
        # it is not a divisor for the texture dimensions.  ``offset_x/y`` are
        # source-pixel offsets and therefore move the rendered image by
        # ``offset * scale`` before rotation.
        fallback_radius = max(width, height) / 2.0 if max(width, height) > 0 else 1.0
        try:
            base_radius = float(metadata.get("radius", fallback_radius))
        except (TypeError, ValueError):
            base_radius = fallback_radius
        if not math.isfinite(base_radius) or base_radius <= 0:
            base_radius = fallback_radius

        def metadata_float(key: str) -> float:
            try:
        ...
        return AssetInfo(
            texture=texture,
            path=path,
            width=width or fallback_radius * 2,
            height=height or fallback_radius * 2,
            base_radius=base_radius,
            offset_x=metadata_float("offset_x"),
            offset_y=metadata_float("offset_y"),
            draw_mode=str(metadata.get("draw_mode", "") or ""),
        )
...
def _normalize_angle(angle: float) -> float:
    """Normalize radians to Wonderdraft's conventional -pi..pi range."""
...
def _matrix_svg_text(matrix: Matrix) -> str:
    return "matrix(" + " ".join(f"{value:.12g}" for value in matrix) + ")"


def _rotation_about(angle: float, center: tuple[float, float]) -> Matrix:
    if not angle:
        return IDENTITY
    cos_a, sin_a = math.cos(angle), math.sin(angle)
    ...
    cx, cy = center
    return _mat_mul(
        _mat_mul((1, 0, 0, 1, cx, cy), rotate),
        (1, 0, 0, 1, -cx, -cy),
    )


def _mirror_about_x(x: float) -> Matrix:
    return (-1.0, 0.0, 0.0, 1.0, 2.0 * x, 0.0)
    ...
    *,
    ...
    if mask is not None:
        ...
        if info:
            ...
            write_png_streaming(
                mask_png, info[0], info[1], info[2], info[4],
                progress=(lambda done, total: progress("Exporting mask PNG", done, total))
    ...
    try:
        with destination.open("wb", buffering=1024 * 1024) as handle:
            ...
            root_attrs = {
                ...
                "wd:format-version": "2", "wd:map-width": f"{width:g}",
            ...
            if mask_png is not None:
                ...
                if embed_background:
                    ...
                    _stream_base64_file(
                        mask_png, handle,
                        progress=(lambda done, total: progress("Embedding mask in SVG", done, total))
                        if progress is not None else None,
                    )
                    _write_xml(handle, '"/>\n')
                else:
            ...
            path_records = root.get("paths", []) or []
            for index, record in enumerate(path_records):
                if progress is not None and (index % max(1, len(path_records) // 200) == 0):
                    progress("Exporting paths", index, len(path_records))
                if not isinstance(record, dict):
                    continue
            ...
            for index, record in enumerate(symbols_data):
                if progress is not None and (index % max(1, len(symbols_data) // 200) == 0):
                    progress("Exporting symbols", index, len(symbols_data))
                if not isinstance(record, dict):
                    continue
                ...
                if asset:
                    # Godot/Wonderdraft draws the source texture at its native
                    # pixel dimensions, then applies the record's scale.  The
                    # logical ``radius`` is not part of the texture-size formula.
                    rendered_width = max(0.001, abs(asset.width * scale[0]))
                    rendered_height = max(0.001, abs(asset.height * scale[1]))
                    offset = _vector2(
                        record.get("offset"), (asset.offset_x, asset.offset_y)
                    )

                    # ``offset`` is expressed in source pixels and is affected by
                    # symbol scale.  Rotation is around the stored map position,
                    # not around the visible texture center.
                    unrotated_visual_center = (
                        center[0] + offset[0] * scale[0],
                        center[1] + offset[1] * scale[1],
                    )
                    x = unrotated_visual_center[0] - rendered_width / 2
                    y = unrotated_visual_center[1] - rendered_height / 2
                    attrs = {
                        **common, "x": f"{x:.9g}", "y": f"{y:.9g}",
                        "width": f"{rendered_width:.9g}", "height": f"{rendered_height:.9g}",
                        "preserveAspectRatio": "none", "xlink:href": asset.path.as_uri(),
                        "filter": f"url(#{filters[rgb_key]})", "opacity": f"{sample_alpha:.6g}",
                        "wd:asset-path": str(asset.path),
                        "wd:source-width": f"{asset.width:.9g}",
                        "wd:source-height": f"{asset.height:.9g}",
                        "wd:base-radius": f"{asset.base_radius:.9g}",
                        "wd:offset-x": f"{offset[0]:.9g}",
                        "wd:offset-y": f"{offset[1]:.9g}",
                        "wd:export-width": f"{rendered_width:.9g}",
                        "wd:export-height": f"{rendered_height:.9g}",
                    }

                    # Mirroring flips the pixels around the visible texture's
                    # center while leaving the Wonderdraft offset anchor in the
                    # same place.  The whole result is then rotated around the
                    # stored map position, matching Sprite2D semantics.
                    transform = IDENTITY
                    if mirror:
                        transform = _mat_mul(
                            transform, _mirror_about_x(unrotated_visual_center[0])
                        )
                    if rotation:
                        transform = _mat_mul(
                            _rotation_about(rotation, center), transform
                        )
                    if transform != IDENTITY:
                        attrs["transform"] = _matrix_svg_text(transform)
                    _write_xml(handle, f"    <image{_xml_attrs(attrs)}/>\n")
                else:
                    missing_symbols += 1
                    offset = _vector2(record.get("offset"), (0.0, 0.0))
                    rotated_offset = _rotate_vector(
                        (offset[0] * scale[0], offset[1] * scale[1]), rotation
                    )
                    visual_center = (
                    ...
                    attrs = {
                        **common, "cx": f"{visual_center[0]:.9g}", "cy": f"{visual_center[1]:.9g}",
                        "r": f"{max(1.0, radius * max(abs(scale[0]), abs(scale[1]))):.9g}",
            ...
            label_records = root.get("labels", []) or []
            for index, record in enumerate(label_records):
                if progress is not None and (index % max(1, len(label_records) // 200) == 0):
                    progress("Exporting labels", index, len(label_records))
                if not isinstance(record, dict):
                    continue
    ...
    if progress is not None:
        progress("SVG export complete", 1, 1)
    return {
...
def _import_symbol(element: ET.Element, matrix: Matrix, root: dict[str, Any], svg_path: Path, resolver: AssetResolver, filters: dict[str, str]) -> dict[str, Any]:
    ...
    if tag == "image":
        visual_center, rendered_width, rendered_height, raw_angle, mirrored = _transformed_rect(element, matrix)
        # For a reflected x-axis the first matrix column points in the
        # opposite direction, so atan2 reports rotation +/- pi.  Undo that
        # convention to recover Wonderdraft's rotation value.
        angle = _normalize_angle(raw_angle + (math.pi if mirrored else 0.0))
    else:
        cx, cy, r = _float_attr(element, "cx"), _float_attr(element, "cy"), _float_attr(element, "r", 1)
        visual_center = _mat_apply(matrix, cx, cy)
        sx, sy, raw_angle, mirrored = _matrix_scale_rotation(matrix)
        angle = _normalize_angle(raw_angle + (math.pi if mirrored else 0.0))
    ...
    original_texture = str(record.get("texture", "")) if record is not None else ""
    if record is None:
        texture = mapped_texture or FALLBACK_TEXTURE
    else:
        texture = original_texture or FALLBACK_TEXTURE
        # A referenced external image must map to a configured asset.  Keep the
        # encoded texture when this is still the image emitted by our exporter:
        # Inkscape commonly rewrites its href to a relative path that no longer
        # resolves from the SVG's new location.  A genuinely replaced unknown
        # image still receives the documented stock-capital fallback.
        if tag == "image":
            if mapped_texture:
                texture = mapped_texture
            elif href and not href.startswith("data:"):
                tagged_texture = _get_wd(element, "texture", "")
                href_stem = Path(urllib.parse.unquote(href)).stem.casefold()
                original_stem = Path(original_texture).name.casefold()
                original_asset = resolver.resolve_texture(original_texture)
                looks_like_original = bool(
                    original_texture
                    and tagged_texture == original_texture
                    and href_stem == original_stem
                    and original_asset is not None
                )
                if not looks_like_original:
                    texture = FALLBACK_TEXTURE

    asset = resolver.asset_info(texture)

    def wd_float(name: str, default: float = 0.0) -> float:
        try:
            value = float(_get_wd(element, name, str(default)) or default)
            return value if math.isfinite(value) else default
        except (TypeError, ValueError):
            return default

    if tag == "image":
        source_width = wd_float("source-width", 0.0)
        source_height = wd_float("source-height", 0.0)
        if source_width <= 0 and asset:
            source_width = asset.width
        if source_height <= 0 and asset:
            source_height = asset.height
    else:
        source_width = source_height = 0.0

    if record is None:
        if asset and source_width > 0 and source_height > 0:
            symbol_scale = (
                rendered_width / source_width,
                rendered_height / source_height,
            )
            offset = (asset.offset_x, asset.offset_y)
            radius = asset.base_radius
        else:
            radius = max(rendered_width, rendered_height) / 2
            symbol_scale = (
                rendered_width / max(1e-6, radius * 2),
                rendered_height / max(1e-6, radius * 2),
            )
            offset = (0.0, 0.0)
        ...
        position = (
            visual_center[0] - rotated_offset[0],
            visual_center[1] - rotated_offset[1],
        )
        record = _default_symbol(position, radius, texture, sample)
        record["offset"] = GDValue("Vector2", offset)
        record["scale"] = GDValue("Vector2", symbol_scale)
        record["type"] = _infer_symbol_type(texture)
    else:
        record["texture"] = texture
        old_scale = _vector2(record.get("scale"), (1, 1))
        old_position = _vector2(record.get("position"))
        old_offset = _vector2(record.get("offset"), (0, 0))
        texture_changed = bool(original_texture and texture != original_texture)

        if texture_changed and asset:
            offset = (asset.offset_x, asset.offset_y)
            record["offset"] = GDValue("Vector2", offset)
            record["radius"] = asset.base_radius
        else:
            offset = old_offset

        if tag == "circle":
            # A fallback circle has no intrinsic sprite dimensions.  Preserve
            # the existing logical radius where possible and express edits as
            # scale, matching real sprite records.
            old_radius = max(1e-6, float(record.get("radius", 16.0)))
            symbol_scale = (
                rendered_width / (2 * old_radius),
                rendered_height / (2 * old_radius),
            )
        elif source_width > 0 and source_height > 0:
            legacy_source_missing = wd_float("source-width", 0.0) <= 0
            old_export_width = wd_float("export-width", 0.0)
            old_export_height = wd_float("export-height", 0.0)
            legacy_unchanged_size = (
                legacy_source_missing
                and old_export_width > 0
                and old_export_height > 0
                and abs(rendered_width / old_export_width - 1.0) < 0.002
                and abs(rendered_height / old_export_height - 1.0) < 0.002
            )
            if legacy_unchanged_size and not texture_changed:
                # Format-version 1 centered symbols on map position and used
                # radius-derived dimensions.  Preserve an untouched old SVG's
                # record exactly rather than interpreting that historical bug
                # as a user resize.
                symbol_scale = old_scale
                position = (
                    old_position[0] + visual_center[0] - old_position[0],
                    old_position[1] + visual_center[1] - old_position[1],
                )
                record["position"] = GDValue("Vector2", position)
                record["scale"] = GDValue("Vector2", symbol_scale)
                record["rotation"] = angle
            ...
            old_width = wd_float("export-width", 0.0)
            old_height = wd_float("export-height", 0.0)
            symbol_scale = old_scale
            if old_width > 0:
                symbol_scale = (old_scale[0] * rendered_width / old_width, symbol_scale[1])
            if old_height > 0:
                symbol_scale = (symbol_scale[0], old_scale[1] * rendered_height / old_height)
...
class WonderdraftEditor(tk.Tk):
    def __init__(self):
        ...
        self.binary_blobs: OrderedDict[str, GDValue] = OrderedDict()
    ...
    def _build_ui(self):
        ...
        ttk.Label(progress_frame, textvariable=self.progress_text, width=34).pack(side="left", padx=(0, 8))
        self.progress_bar = ttk.Progressbar(progress_frame, mode="determinate", maximum=100.0)
        self.progress_bar.pack(side="left", fill="x", expand=True)
        self.progress_percent = tk.StringVar(value="")
        ttk.Label(progress_frame, textvariable=self.progress_percent, width=9, anchor="e").pack(side="right", padx=(8, 0))
    ...
    def _progress_start(self, action: str, total: int | None = None) -> None:
        self.progress_text.set(action)
        self.progress_percent.set("")
        self._progress_last_ui = 0.0
        if total is None or total <= 0:
            self.progress_bar.stop()
            self.progress_bar.configure(mode="indeterminate", maximum=100.0, value=0)
            self.progress_bar.start(12)
        else:
            self.progress_bar.stop()
            self.progress_bar.configure(mode="determinate", maximum=100.0, value=0)
        self.status.set(action)
        self.update_idletasks()

    def _status_progress(self, action: str, done: int, total: int) -> None:
        now = time.monotonic()
        if total > 0:
            percent = max(0.0, min(100.0, 100.0 * done / total))
            self.progress_bar.stop()
            self.progress_bar.configure(mode="determinate", maximum=100.0, value=percent)
            self.progress_percent.set(f"{percent:5.1f}%")
            self.progress_text.set(
                f"{action} — {done / (1024**2):,.1f}/{total / (1024**2):,.1f} MiB"
            )
            self.status.set(f"{action}: {percent:5.1f}%")
        else:
            self.progress_text.set(action)
            self.progress_percent.set("")
            self.status.set(action)
        if now - self._progress_last_ui >= 0.03 or done >= total:
            self._progress_last_ui = now
            self.update_idletasks()

    def _count_progress(self, action: str, done: int, total: int) -> None:
        if total <= 0:
            self._progress_start(action, None)
            return
        percent = max(0.0, min(100.0, 100.0 * done / total))
        self.progress_bar.stop()
        self.progress_bar.configure(mode="determinate", maximum=100.0, value=percent)
        self.progress_percent.set(f"{percent:5.1f}%")
        self.progress_text.set(f"{action} — {done:,}/{total:,}")
        self.status.set(f"{action}: {percent:5.1f}%")
        now = time.monotonic()
        if now - self._progress_last_ui >= 0.03 or done >= total:
            self._progress_last_ui = now
    ...
    def _progress_finish(self, message: str) -> None:
        self.progress_bar.stop()
        self.progress_bar.configure(mode="determinate", maximum=100.0, value=100.0)
        self.progress_percent.set("100.0%")
        self.progress_text.set(message)
        self.status.set(message)
        self.update_idletasks()

    def _progress_fail(self, message: str) -> None:
        self.progress_bar.stop()
        self.progress_bar.configure(mode="determinate", maximum=100.0, value=0)
        self.progress_percent.set("")
        self.progress_text.set(message)
    ...
    def _set_text_root(self, root: dict[str, Any], work_dir: Path | None = None) -> None:
        """Serialize via a temporary UTF-8 file, then load it with exact progress."""
        directory = work_dir or Path(tempfile.gettempdir())
        directory.mkdir(parents=True, exist_ok=True)
        handle = tempfile.NamedTemporaryFile(
            prefix="wonderdraft_editable_", suffix=".txt", dir=directory, delete=False
        )
        text_path = Path(handle.name)
        handle.close()
        try:
            self._progress_start("Serializing editable map text", None)
            written = 0
            next_report = 0
            with text_path.open("wb", buffering=1024 * 1024) as output:
                for piece in iter_godot_text(root):
                    ...
                    if written >= next_report:
                        self.progress_text.set(
                            f"Serializing editable map text — {written / (1024**2):,.1f} MiB"
                        )
                        self.update_idletasks()
            ...
            self._progress_start("Loading editable map text", written)
            self.text.configure(undo=False)
            self.text.delete("1.0", "end")
            loaded = 0
            with text_path.open("r", encoding="utf-8", buffering=1024 * 1024) as source:
                while True:
                    chunk = source.read(1024 * 1024)
                    if not chunk:
                        break
                    self.text.insert("end", chunk)
                    loaded += len(chunk.encode("utf-8"))
                    self._status_progress("Loading editable map text", loaded, written)
    ...
    def open_map(self):
        ...
        try:
            self._progress_start("Creating disk-backed map cache", None)
            ...
            self._progress_start("Decoding map metadata", meta.get("uncompressed_size", 0))
            root, _ = decode_store_var_file(
                stream_path,
                lambda done, total: self._status_progress("Decoding map metadata", done, total),
            )
            if not isinstance(root, dict):
                raise FormatError("The map root is not a Godot Dictionary")
            self._progress_start("Preparing editable map structure", None)
            editable, images, binary_blobs = prepare_editable_tree(root)
            self._set_text_root(editable, Path(new_temp.name))
            ...
            self.images = images
            self.binary_blobs = binary_blobs
            ...
            self.status.set(
                f"Loaded {self.current_path.name} — {len(self.images)} images and "
                f"{len(self.binary_blobs)} additional binary payloads are disk-backed; "
                f"{raw_gib:,.2f} GiB cache"
            )
            self._progress_finish(self.status.get())
            del root, editable
            gc.collect()
        except Exception as exc:
            ...
            self._progress_fail("Open failed")

    def validate_text(self, quiet=False):
        try:
            text_value = self.text.get("1.0", "end")
            self._progress_start("Parsing editable map text", len(text_value))
            parsed = parse_godot_text(
                text_value,
                progress=lambda done, total: self._status_progress(
                    "Parsing editable map text", done, total
                ),
            )
            del text_value
            ...
            if not quiet:
                messagebox.showinfo("Valid", "The map text is syntactically valid.")
            self._progress_finish("Text validated successfully")
            return parsed
        except Exception as exc:
            if not quiet:
                messagebox.showerror("Invalid map text", str(exc))
            self._progress_fail("Validation failed")
            return None
    ...
    def save_map(self):
        ...
        if not filename:
            return
        try:
            self._progress_start("Streaming map to compressed output", None)
            root = restore_external_payloads(parsed, self.images, self.binary_blobs)
            destination = Path(filename)
            packed_size = write_wonderdraft_map_streaming(
                root, destination, self.block_size,
                compressed=self.use_compression.get(),
                progress=lambda done, total: self._status_progress(
                    "Writing compressed map", done, total
                ),
            )
            verified = False
            if self.verify_saved_map.get():
                self._progress_start("Verifying saved map", None)
                cache_root = Path(self.settings.get("cache_folder") or detect_cache_folder()).expanduser()
                cache_root.mkdir(parents=True, exist_ok=True)
                with tempfile.TemporaryDirectory(prefix="wonderdraft_verify_", dir=cache_root) as verify_dir:
                    ...
                    decompress_gcpf_path(
                        destination, verify_path,
                        lambda done, total: self._status_progress(
                            "Verifying: decompressing", done, total
                        ),
                    )
                    check_root, _ = decode_store_var_file(
                        verify_path,
                        lambda done, total: self._status_progress(
                            "Verifying: decoding", done, total
                        ),
                    )
            ...
            self._progress_finish(
                f"Saved {destination.name} ({packed_size:,} bytes, {mode}{verification})"
            )
            messagebox.showinfo(
                "Saved",
            ...
            self._progress_fail("Save failed")
    ...
    def export_svg(self):
        ...
        try:
            ...
            self._progress_start(f"Streaming SVG export ({mode})", None)
            summary = export_svg_file(
                ...
                progress=lambda action, done, total: self._count_progress(action, done, total)
                if total < 1024 * 1024 else self._status_progress(action, done, total),
            )
            self._progress_finish(
            ...
            self._progress_fail("SVG export failed")
    ...
    def import_svg(self):
        ...
        try:
            self._progress_start("Importing SVG", None)
            updated, summary = import_svg_file(parsed, Path(filename), self.asset_resolver)
            work_dir = Path(self._map_temp.name) if self._map_temp is not None else None
            self._set_text_root(updated, work_dir)
            warning_text = ""
            warnings = summary.get("warnings", [])
            if warnings:
                warning_text = "\n\nWarnings:\n" + "\n".join(str(w) for w in warnings[:10])
            self._progress_finish(
                f"Imported SVG: {summary['labels']} labels, {summary['symbols']} symbols, {summary['paths']} paths"
            )
            ...
            self._progress_fail("SVG import failed")
    ...
    def export_selected_image(self):
        ...
        if not filename:
            return
        try:
            self._progress_start(f"Streaming {key} to PNG", _source_length(info[4]))
            write_png_streaming(
                Path(filename), info[0], info[1], info[2], info[4],
                progress=lambda done, total: self._status_progress(
                    f"Exporting {key}", done, total
                ),
            )
            self._progress_finish(f"Exported {key}")
        except Exception as exc:
    ...
    def export_all_images(self):
        ...
        try:
            ...
            for key, image_obj in self.images.items():
                ...
                if info:
                    valid_images.append((key, info))
                    total_bytes += info[0] * info[1] * {"L8": 1, "LA8": 2, "RGB8": 3, "RGBA8": 4}.get(info[2], 4)
            completed = 0
            self._progress_start("Exporting embedded PNGs", total_bytes)
            for index, (key, info) in enumerate(valid_images, start=1):
                image_size = info[0] * info[1] * {"L8": 1, "LA8": 2, "RGB8": 3, "RGBA8": 4}.get(info[2], 4)
                write_png_streaming(
                    Path(directory) / f".{key.split('.')[-1]}.png",
                    info[0], info[1], info[2], info[4],
                    progress=lambda done, _total, base=completed, name=key: self._status_progress(
                        f"Exporting image {index}/{len(valid_images)}: {name}",
                        base + done, total_bytes,
                    ),
                )
                completed += image_size
                exported += 1
            self._progress_finish(f"Exported {exported} PNG files with bounded memory")
        except Exception as exc:
            messagebox.showerror("Export failed", str(exc))
            self._progress_fail("PNG export failed")
